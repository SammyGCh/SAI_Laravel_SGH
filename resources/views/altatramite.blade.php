...
<section id="wrapper">
        <div class="contenido">
                <div id="formulario">
                    <form action="{{url('altatramite/registro')}}" method="POST">
                        {{ csrf_field() }}
                        ...
                        <div>
                            <label for="status" class="form-Nombres">Tramite (*)</label>
                            <select name="tipoTramite" required class="form-Menu">
                                <option disabled selected value="0">Tipo de tramite</option>
                                @foreach($all_tipoTramite_info as $tipoTramite)
                                <option value="{{$tipoTramite->idtipoTramite}}" >{{$tipoTramite->categoria}}</option>
                                @endforeach
                            </select>
                        </div><br>
